...
async def test_digest_prefetch_topk(mock_papers, monkeypatch):
    ...
    with (
        ...
        patch(
            "src.api.routers.digest.translate_to_japanese",
            side_effect=lambda x, _: f"Translated: {x}",
        ),
        patch(
            "src.api.routers.digest.make_short_summary", return_value="Short summary"
        ),
        patch("src.api.routers.digest.get_cached", return_value=None),
        patch("src.api.routers.digest.mcp_download_paper", return_value=True),
        patch("src.api.routers.digest.mcp_read_paper", return_value="Test content"),
        patch("src.api.routers.digest.set_cached"),
        patch(
            "src.api.routers.digest.build_sections",
            return_value={"sections": [], "toc_flat": []},
        ),
...
async def test_digest_prefetch_with_cache_hit(mock_papers, monkeypatch):
    ...
    with (
        ...
        patch("src.api.routers.digest.mcp_download_paper"),
        patch("src.api.routers.digest.mcp_read_paper"),
    ):
        result = await get_digest(cat="cs.AI", days=1, limit=5)
