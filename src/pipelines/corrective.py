...
def answer_with_correction(question: str, theta: float = None, index=None) -> AnswerResult:
    """
...
def no_answer(question: str, attempts: list = None) -> AnswerResult:
    ...
    no_answer_text = "この質問に答えるには、以下の情報が不足している可能性があります：\n\n"
...
def analyze_missing_elements(question: str) -> list:
    ...
    time_indicators = [
        "year", "years", "年", "最近", "recent", "latest", "新しい", "古い", 
        "2023", "2024", "2025", "since", "before", "after"
    ]
    has_time = any(indicator in question_lower for indicator in time_indicators)
    
    # Check for method/algorithm indicators  
    method_indicators = [
        "algorithm", "method", "アルゴリズム", "手法", "model", "モデル",
        "approach", "technique", "framework", "architecture", "transformer",
        "bert", "gpt", "neural", "deep learning", "machine learning"
    ]
    has_method = any(indicator in question_lower for indicator in method_indicators)
    
    # Check for dataset indicators
    dataset_indicators = [
        "dataset", "data", "データセット", "データ", "benchmark", "corpus",
        "imagenet", "coco", "glue", "squad", "evaluation"
    ]
    has_dataset = any(indicator in question_lower for indicator in dataset_indicators)
    
    # Check for domain indicators
    domain_indicators = [
        "computer vision", "nlp", "natural language", "画像", "テキスト", 
        "speech", "音声", "robotics", "ロボット", "medical", "医療"
    ]
    ...
    if not has_method:
        missing_elements.append("特定の手法やアルゴリズム名（例：Transformer、BERT、CNNなど）")
    
    if not has_dataset:
        missing_elements.append("対象となるデータセットやベンチマーク（例：ImageNet、GLUE、CoCoなど）")
    
    if not has_domain:
        missing_elements.append("研究領域の詳細（例：コンピュータビジョン、自然言語処理、音声認識など）")
